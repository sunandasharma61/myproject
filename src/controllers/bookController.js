...
const getWithYear= async function (req, res) {
    let yearBook= req.body.year
    let yearBooks= await BookModel.find({ year:yearBook})
...
const getWithPrice= async function (req, res) {
    let allBooks= await BookModel.find({indianPrice:{$or:[" 100INR"," 200INR"," 300INR"]}})
    res.send({msg: allBooks})
}

const getWithPages= async function (req, res) {
    let allBooks= await BookModel.find({totalPages:{$gt:50}})
    res.send({msg: allBooks})
}
...
module.exports.getWithPrice= getWithPrice
